...
class Issue < ActiveRecord::Base
  ...
  scope :recently_updated, :order => "#{self.table_name}.updated_on DESC"
  ...
  after_initialize :set_defaults_to_new_record
  def set_defaults_to_new_record
    if new_record?
  ...
  def self.update_versions(conditions=nil)
    ...
    #          ).each do |issue|
    Issue.joins(:fixed_version).where("#{Issue.table_name}.fixed_version_id IS NOT NULL" +
                                                " AND #{Issue.table_name}.project_id <> #{Version.table_name}.project_id" +
                                                " AND #{Version.table_name}.sharing <> 'system'", conditions
              ).each do |issue|
