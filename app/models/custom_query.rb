...
class CustomQuery < ActiveRecord::Base
  ...
  def available_filters
    ...
    user_values = []
    user_values << ["<< #{I18n.t(:label_me)} >>", "me"] if User.current.logged?
    if project
    ...
    if User.current.logged?
      @available_filters["watcher_id"] = { :type => :list, :order => 15, :values => [["<< #{I18n.t(:label_me)} >>", "me"]] }
    end
