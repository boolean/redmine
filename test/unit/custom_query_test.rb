...
class CustomQueryTest < ActiveSupport::TestCase
  fixtures :projects, :enabled_modules, :users, :members, :member_roles, :roles, :trackers, :issue_statuses, :issue_categories, :enumerations, :issues, :watchers, :custom_fields, :custom_values, :versions, :custom_queries

  def test_custom_fields_for_all_projects_should_be_available_in_global_queries
    query = CustomQuery.new(:project => nil, :name => '_')
  ...
  def test_query_should_allow_shared_versions_for_a_project_query
    ...
    query = CustomQuery.new(:project => Project.find(1), :name => '_')
  ...
  def test_filter_watched_issues
    ...
    query = CustomQuery.new(:name => '_', :filters => { 'watcher_id' => {:operator => '=', :values => ['me']}})
    result = find_issues_with_query(query)
  ...
  def test_invalid_query_should_raise_query_statement_invalid_error
    q = CustomQuery.new
    assert_raise CustomQuery::StatementInvalid do
      q.issues(:conditions => "foo = 1")
    end
  end
  
  def test_issue_count_by_association_group
    q = CustomQuery.new(:name => '_', :group_by => 'assigned_to')
  ...
  def test_issue_count_by_list_custom_field_group
    q = CustomQuery.new(:name => '_', :group_by => 'cf_1')
    count_by_group = q.issue_count_by_group
    assert_kind_of Hash, count_by_group
  ...
  def test_issue_count_by_date_custom_field_group
    q = CustomQuery.new(:name => '_', :group_by => 'cf_8')
    count_by_group = q.issue_count_by_group
    assert_kind_of Hash, count_by_group
